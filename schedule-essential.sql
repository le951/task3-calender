...
# essential용 파일과 challenge용 파일은 schema를 공유한다.
# 이미 만들었다면 생략
CREATE SCHEMA task3;
...
# 테이블 이전
# RENAME TABLE task3_essential.schedules TO task3.schedules_essential;
# DROP DATABASE task3_essential;

# 최초 테이블 생성. 필수 과제용.
CREATE TABLE schedules_essential(
...
# 필수 과제 테이블 테스트용 데이터
INSERT INTO schedules_essential(title, memo, start_time, end_time, writer, password)
    VALUES ('테스트 1', '테스트 1 메모', '2025-03-25', '2025-03-26', 'Lee', '1'),
           ('테스트 2', '테스트 2 메모', '2025-03-24', null, 'H', '2'),
           ('테스트 3', '테스트 3 메모', '2025-03-27', '2025-03-27', 'H', '3');

INSERT INTO schedules_essential(title, memo, start_time, end_time, writer, password)
VALUES ('테스트 4', '테스트 4 메모', '2025-03-25', '2025-03-26', 'Lee', '4'),
       ('테스트 5', '테스트 5 메모', '2025-03-24', null, 'H', '5'),
...
# revision_time 작동 확인
UPDATE schedules_essential SET writer = 'M' WHERE no=6;


# Query문 동작 테스트
SELECT *
FROM schedules_essential
...
UPDATE schedules_essential SET title = 'renewTitle' WHERE no=10 AND password='p1';
UPDATE schedules_essential SET title = 'renew!', memo='renewMemo' WHERE no=10 AND password='p1';

DELETE FROM schedules_essential WHERE no=13;

SELECT EXISTS (SELECT 1 FROM schedules_essential WHERE no > 1);

SELECT no FROM schedules_essential WHERE no=1 LIMIT 1;

# 검색 시 사용되는 start / end time들에 대해 INDEX 설정.
# 생성에서 추가했다면 실행 X.

# CREATE INDEX idx_start_time ON schedules_essential (start_time);
# CREATE INDEX idx_end_time ON schedules_essential (end_time);

# CREATE INDEX idx_times ON schedules_essential (start_time, end_time);
# CREATE INDEX idx_revision_time ON schedules_essential (revision_time);

# NOT NULL 설정 추가
ALTER TABLE schedules_essential
