...
public class SchedulerRepositoryImpl implements SchedulerRepository {
    ...
    public long createSchedule(CreateScheduleDTO dto) {
        SimpleJdbcInsert insert = new SimpleJdbcInsert(jdbcTemplate)
                .withTableName("schedules_essential") // INSERT 할 테이블
...
//        log.info(insert.getColumnNames().toString());
    ...
    public List<ResponseSchedulesDTO> getSchedules(RequestSchedulesDTO dto) {
        ...
        StringBuilder query = new StringBuilder("SELECT no, title, start_time, end_time FROM schedules_essential " +
                "WHERE true");
...
//        }
    ...
    public ResponseScheduleDTO getSchedule(long l) {
        String query = "SELECT * FROM schedules_essential WHERE no=?";
    ...
    public Boolean patchSchedule(PatchScheduleDTO dto) {
        String query = "UPDATE schedules_essential SET writer=?, title=?, memo=? WHERE no=? AND password=?";
    ...
    public Boolean deleteSchedule(DeleteScheduleDTO dto) {
        ...
        String query = "SELECT no FROM schedules_essential WHERE no=? LIMIT 1";
...
//                ps.setString(5,dto.getPassword());
        ...
        query = "DELETE FROM schedules_essential WHERE no=? AND password=?";
